...
def _fetch_hs300_index_recent(days: int = 14):
    ...
    try:
        for index_code in ['000300', '000905']:
            try:
                df = fetch_hs300_index.fetch_index_range(index_code, start, end, chunk_days=180, retries=3)
            except Exception as ex:
                print(f'东方财富抓取失败({index_code})，切换备用源: {type(ex).__name__}: {ex}')
                try:
                    df = fetch_hs300_index.fetch_index_via_stock_hist(index_code, start, end)
                    if df is None or df.empty:
                        raise RuntimeError('stock_zh_a_hist returned empty')
                except Exception as ex2:
                    print(f'个股历史接口抓取失败({index_code})，切换备用源: {type(ex2).__name__}: {ex2}')
                    df = fetch_hs300_index.fetch_index_fallback(index_code, start, end)
            inserted, updated = fetch_hs300_index.save_to_db(db, df, index_code=index_code)
